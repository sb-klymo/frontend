...
function makeRequest(body: unknown = { conversation_id: "conv-uuid-123" }): Request {
  return new Request("http://test/api/chat/resume-approval", {
    method: "POST",
...
describe("POST /api/chat/resume-approval BFF route", () => {
  ...
  it("forwards 204 from backend as 204 (nothing to resume)", async () => {
    authedSession("tok-resume");
    vi.mocked(global.fetch).mockResolvedValue(new Response(null, { status: 204 }));

    const response = await POST(makeRequest());

    expect(response.status).toBe(204);
    // Verify the upstream call was made to the correct path with correct auth.
    expect(global.fetch).toHaveBeenCalledWith(
      "http://backend-test/chat/resume-approval",
      expect.objectContaining({
        method: "POST",
        headers: expect.objectContaining({
          Authorization: "Bearer tok-resume",
  ...
  it("pipes 200 SSE stream back with correct headers", async () => {
    authedSession();
    const ssePayload = `data: {"type":"message","content":"Votre réservation est approuvée."}\n\n`;
    vi.mocked(global.fetch).mockResolvedValue(
    ...
    const text = await response.text();
    expect(text).toContain("approuvée");
  });
