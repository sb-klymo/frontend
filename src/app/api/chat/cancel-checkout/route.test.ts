...
function makeRequest(body: unknown = { conversation_id: "conv-uuid-123" }): Request {
  return new Request("http://test/api/chat/cancel-checkout", {
    method: "POST",
...
describe("POST /api/chat/cancel-checkout BFF route", () => {
  ...
  it("forwards 204 from backend as 204 (nothing to cancel)", async () => {
    authedSession("tok-cancel");
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      "http://backend-test/chat/cancel-checkout",
  ...
  it("pipes 200 SSE stream back with correct headers", async () => {
    authedSession();
    const ssePayload = `data: {"type":"message","content":"Votre réservation a été annulée."}\n\n`;
    vi.mocked(global.fetch).mockResolvedValue(
